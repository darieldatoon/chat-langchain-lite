...
def render_trace_link(url: str | None) -> None:
    """Render a right-aligned, small link to the run's LangSmith trace.

    The URL is system-generated by LangSmith (always https://smith.langchain.com/…),
    not user/model input — the startswith guard keeps this anchor injection-safe.
    """
    if not url or not url.startswith("https://"):
        return
    st.markdown(
        f'<div class="trace-link"><a href="{url}" target="_blank">View trace in LangSmith ↗</a></div>',
...
<style>
  ...
  * { font-family: 'Inter', sans-serif !important; box-sizing: border-box; }
  ...
  /* Trace link under each assistant response */
  .trace-link { text-align: right; margin-top: 2px; margin-bottom: 12px; }
  .trace-link a {
    color: #52525b !important;
    font-size: 11px !important;
    text-decoration: none !important;
  }
  .trace-link a:hover { color: #93c5fd !important; }
</style>
""", unsafe_allow_html=True)
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"], avatar="🧑" if msg["role"] == "user" else "💬"):
        ...
        render_trace_link(msg.get("trace_url"))
        ...
        try:
            ...
            while True:
                try:
                    ...
                    trace_url = stop.value
                    break
        except Exception as e:
            response = f"Error: {e}"
            placeholder.markdown(response)

        render_trace_link(trace_url)

    st.session_state.messages.append(
        {"role": "assistant", "content": response, "trace_url": trace_url}
    )
    st.rerun()
